...
if(!($WildCardSecretCheck = Get-AzKeyVaultSecret -VaultName $SolutionKeyVault -Name "%name%-wildcard-certf*"))
{
...
Write-Output "Migrate KeyVault secret"

$File =  $pfxpath+"WildCard.blob"

$WildCardSecret = Get-AzKeyVaultSecret -VaultName $WildCardKeyVaultName -Name "%name%-wildcard-certf*"


Write-Output "Backup KeyVault secret"

Backup-AzKeyVaultSecret -VaultName $WildCardKeyVaultName -Name $WildCardSecret.Name -OutputFile $File -Force

Write-Output "Restore KeyVault secret"

Restore-AzKeyVaultSecret -VaultName $SolutionKeyVault -InputFile $File

}
else 
{

Write-Output "KeyVault secret $WildCardSecretCheck already exists"  
