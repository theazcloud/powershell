...
##############################################################################
#  This script requires the PowerShell Az modules, you will need to run      #  
#                                                                            #
#  Install-Module Az                                                         #
#  Then Import-Module Az                                                     #
#                                                                            #
##############################################################################

##############################################################################
##                                                                          ##
##      Complete the variables below from the details in the portal         ##
##                                                                          ##
##############################################################################
#                                                                            # 
########################## Certifcate name goes here #########################
...
################# Subscription containing the certificate ####################

$SubscriptionId = ""

##############################################################################

########## Name and file path of the exported PFX file #######################

$pfxname = ""

#################### Create folder if it does not exist or change path #######

$pfxpath = "C:\AzureExport"

############################################################################## 
##                                                                          ##
##                 End of variables to be completed                         ##
##                                                                          ##
##############################################################################
...
################################################################ DEFINES FUNCTION TO EXPORT APP SERVICE CERTIFICATE  ################################################################################################
...
$ascResource= Get-AzResource -ResourceId "/subscriptions/$subscriptionId/resourceGroups/$resourceGroupName/providers/Microsoft.CertificateRegistration/certificateOrders/$Certname"
...
################ This block is the change from the original script which is the workaround for the PowerShell breaking change ###########
...
[io.file]::WriteAllBytes(".\$pfxname.pfx",$pfxCertObject.Export([System.Security.Cryptography.X509Certificates.X509ContentType]::Pkcs12,$pfxPassword))
...
## Remove comments to run the command below if you are sure that the account is not used for any other access on the KeyVault or login to the portal and change the access policy accordingly.

# Remove-AzKeyVaultAccessPolicy -VaultName $keyVaultName -UserPrincipalName $loginId -PassThru
...
Write-Host "Created an App Service Certificate copy at: $currentDirectory\$pfxname.pfx"
...
#######################################################################################################
Write-Host "Secret Access to account $loginId has been removed from the KeyVault"


Export-AppServiceCertificate -loginId $loginId -subscriptionId $SubscriptionId -resourceGroupName $ResourceGroupName -name $CertName